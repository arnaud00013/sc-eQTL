...
for current_chromosome in lst_label_chromosomes:
    ...
    dict_HMM_posterior_probs[current_chromosome] = np.zeros(shape=(nb_cells,num_SNPs[i]))
    i = i + 1
...
# loop over barcodes
for i in np.arange(len(lst_cells)):
    #Get coverage stats
    all_reads_RM = 0
    all_reads_BY = 0
    total_len = 0
    total_nonzeros = 0
    for current_chromosome in lst_label_chromosomes:
        all_reads_RM = all_reads_RM + np.nansum(dict_RM_count[current_chromosome][i,:])
        all_reads_BY = all_reads_BY + np.nansum(dict_BY_count[current_chromosome][i,:])
        total_len = total_len + len(dict_RM_count[current_chromosome][i,:])
        total_nonzeros = np.count_nonzero(dict_RM_count[current_chromosome][i,:]+dict_BY_count[current_chromosome][i,:])
        
    coverage = float(total_nonzeros)/float(total_len)
    total_avg_reads = (all_reads_RM+all_reads_BY)/float(total_len)
    print("Barcode {0} average depth of coverage = {1} and breadth of coverage = {2}!".format(lst_cells[i],total_avg_reads,coverage))
    
    id_chrom = 0
    # Loop over chromosomes for HMM
    for current_chromosome in lst_label_chromosomes:
        # Pick out reads for this current_chromosome
        all_reads_RM = dict_RM_count[current_chromosome][i,:]
        all_reads_BY = dict_BY_count[current_chromosome][i,:]
        ...
        if np.isnan(O).any():
            print >> sys.stderr, 'Error! nan in observation probabilites, '+str(current_chromosome)+'. '
            #sys.stderr.flush()
            print(i)     
            break    
        ...
        if np.isnan(fprobs_norm).any():
            print >> sys.stderr, 'Error! nan in fprobs, '+str(current_chromosome)+'. '
            #sys.stderr.flush()
            print(i)
            break    
        if np.isnan(bprobs_norm).any():
            print >> sys.stderr, 'Error! nan in bprobs, '+str(current_chromosome)+'. '
            #sys.stderr.flush()
            print(i)
            break    
        ...
        if np.isnan(post_probs).any():
            print >> sys.stderr, 'Error! nan in inferred genome, '+str(current_chromosome)+'. '
            #sys.stderr.flush()    
            print(i)
            break    
        #save HMM genotype
