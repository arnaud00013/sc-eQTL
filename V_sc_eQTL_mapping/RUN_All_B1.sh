...
#SBATCH -J RUN_All_B1_sc_eQTL # Job name
...
do
  ...
  sbatch --array=$start_gene_id-$end_gene_id RUN_B1_SC_EQTL.sh
  nb_running_processes=$(squeue | nl | grep p1211536 | wc -l)
  while [ "$limit_nb_proc" -lt "$nb_running_processes" ]
  do
    nb_running_processes=$(squeue | nl | grep p1211536 | wc -l)
  done
  #nb_lines_out_B1=$(wc -l lst_out_B1_p2.txt | awk '{print $1}')
  #while [ $nb_lines_out_B1 -lt $current_uplimit_gene_id ]
  ...
  #do
    #sleep 30s
    #nb_completed_ocvs=$(grep -l "find_new    50" gene_id_{$start_gene_id..$end_gene_id}/oCV_*/output.txt | wc -l)
    #ls gene_id*/B1_done.txt > lst_out_B1_p2.txt
    #nb_lines_out_B1=$(wc -l lst_out_B1_p2.txt | awk '{print $1}')
    #printf '%s\n' "$nb_lines_out_B1"
  #done
...
#do
#  sbatch --array=$start_gene_id-$end_gene_id RUN_B1_SC_EQTL.sh
#  nb_lines_out_B1=$(wc -l lst_out_B1.txt | awk '{print $1}')
#  while [ $nb_lines_out_B1 -lt $current_uplimit_gene_id ]
#  do
#    sleep 5m
#    ls gene_id*/B1_done.txt > lst_out_B1.txt
#    nb_lines_out_B1=$(wc -l lst_out_B1.txt | awk '{print $1}')
#  done
