...
optional.add_argument('--nosingle', help='No single effects', action='store_true')
optional.add_argument('--noepistasis', help='No epistatic effects', action='store_true')
...
# Now parse the positions into single effects and epistasis
positions = np.array(re.split("[\s\t]",pos_line))
effects = np.loadtxt(str(args.i), skiprows = 1)

#print(np.where(np.char.find(positions,",") >= 0))
#train_set[~np.isnan(train_set)]
if(args.noepistasis):
	positions = np.take(positions,np.where(np.char.find(positions,",") < 0))[0]

if(args.nosingle):
	positions = np.take(positions,np.where(np.char.find(positions,",") >= 0))[0]

if(positions.size == 0):
	print("No positions found or all filtered.", file=sys.stderr)

# Load the fitness data
mtx_gene_expression = pd.read_csv(args.fit,sep="\t",header=None).to_numpy()
#mtx_gene_expression = (mtx_gene_expression - mtx_gene_expression.mean(axis=0))/np.std(mtx_gene_expression,axis=0) #mtx_gene_expression = mtx_gene_expression/np.nansum(a=mtx_gene_expression,axis=1, keepdims=True)
'''
#Median expression for barcodes with same reference lineage assignment
...
if(args.model != 2):
	...
	validation_perm = np.take(perm, np.r_[outside_CV/10 * len(fitnesses):(outside_CV + 1)/10 * len(fitnesses)].astype(int))
...
for the_label_chr in lst_label_chromosomes[0:len(lst_label_chromosomes)]:
    genotypes_file.append(genotypes_file.append(np.transpose(max_reads_G[:,lst_inds_begin_chrs[i]:(lst_inds_finish_chrs[i]+1)]))) #np.transpose(mtx_corrected_imputed_genotypes[:,lst_inds_begin_chrs[i]:(lst_inds_finish_chrs[i]+1)])
...
for pos_index in range(len(positions)):
	...
	if("," in positions[pos_index]):
		# in epistasis
		# remove bracket
		epistatic_term = positions[pos_index][1:]
		...
		X_train[:,pos_index] = epis_pos_line_train.copy()
		X_test[:,pos_index] = epis_pos_line_test.copy()

		X_train_2[:,pos_index] = epis_pos_line_train * np.sqrt(train_errors)
...
# reoptimize the effects


#initial_beta = effects

predicted_phenotypes = np.dot(X_test, initial_beta_2)
RSS = np.sum((test_phenotypes - np.mean(test_phenotypes) - predicted_phenotypes + np.mean(predicted_phenotypes))**2)
R2 = 1-RSS/TSS

predicted_phenotypes_2 = np.dot(X_train, initial_beta_2)
RSS_2 = np.sum((train_phenotypes - np.mean(train_phenotypes) - predicted_phenotypes_2 + np.mean(predicted_phenotypes_2))**2)
R2_2 = 1-RSS_2/TSS_2


var = RSS_2 / (train_num_usable_spores - (len(positions)+1))

# Now get (XtX)^-1

fisher_information = np.diag(np.linalg.inv(np.dot(np.transpose(X_train_2), X_train_2)))

errors = 1.96 * np.sqrt(var*fisher_information)

for i in range(len(initial_beta_2)):
	print(str(positions[i]) + "	" + str(initial_beta_2[i]) + "	" + str(errors[i]))
