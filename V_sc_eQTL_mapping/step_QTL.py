...
optional.add_argument('--nosave', help='Set to 1 to avoid saving the npy progress files.', default=1, type=int)
optional.add_argument('--maxqtl', help='Number of QTLs to find.', default=2500, type=int)
...
def refine_positions(num):
    ...
    for scan_pos in range(left_bracket + num, right_bracket + 1, poolcount):
        if (scan_pos-start_index) >= len(genotypes_file[chr_of_snp-1]): #scaning within chromosome
            continue
