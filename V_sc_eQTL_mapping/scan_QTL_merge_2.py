...
required.add_argument('-k', help='Value of the L0 regularization parameter', default=0, type=float)
...
for i in range(len(AICs)):
	pos = positions[i]
	likelihood = AICs[i] + len(pos) * args.k

	if(likelihood < min_likelihood):
		best_pos = pos
		best_effects = effects[i]
...
X_train_2 = np.ones((train_num_usable_spores,len(best_pos)+1))
X_test = np.ones((test_num_usable_spores,len(best_pos)))
...
X_train_2[:,len(best_pos)] = X_train_2[:,len(best_pos)] * np.sqrt(train_errors)
...
initial_beta_2 = beta[0:len(best_pos)]
...
alphas = np.linspace(0.20,1,1600)
