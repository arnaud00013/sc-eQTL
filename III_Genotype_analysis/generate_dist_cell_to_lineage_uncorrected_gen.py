...
#matrix of original RM ratio (uncorrected RM ratios)
df_uncorrected_RM_ratio = pd.read_csv("{0}/data/good_cells_genotypes/RM_ratio_{1}.csv".format(workspace_path,lst_label_chromosomes[0]),sep="\t",header=0,index_col=0)
for the_label_chr in lst_label_chromosomes[1:16]:
    df_uncorrected_RM_ratio = pd.concat([df_uncorrected_RM_ratio,pd.read_csv("{0}/data/good_cells_genotypes/RM_ratio_{1}.csv".format(workspace_path,the_label_chr),sep="\t",header=0,index_col=0)],axis=1)
mtx_uncorrected_RM_ratio = df_uncorrected_RM_ratio.to_numpy()
#mtx_uncorrected_RM_ratio = np.round(mtx_uncorrected_RM_ratio)
...
#Barcode HMM imputation uncertainty score
arr_barcodes_uncertainty_scores = np.zeros(np.shape(mtx_corrected_imputed_genotypes)[0])
for ind_barcode in np.arange(np.shape(mtx_corrected_imputed_genotypes)[0]):
    arr_barcodes_uncertainty_scores[ind_barcode] = (4/np.shape(mtx_corrected_imputed_genotypes)[1])*np.sum(mtx_corrected_imputed_genotypes[ind_barcode,:]*(1-(mtx_corrected_imputed_genotypes[ind_barcode,:])))
#save metadata about selected expressed genes names
data = {'barcode' : barcodes,
        'uncertainty_score' : arr_barcodes_uncertainty_scores.tolist()}
df_barcodes_uncertainty_scores = pd.DataFrame(data)
df_barcodes_uncertainty_scores["rank"] = df_barcodes_uncertainty_scores["uncertainty_score"].rank()
df_barcodes_uncertainty_scores.to_csv("{0}/df_barcodes_uncertainty_scores.csv".format(workspace_path), sep='\t',na_rep="NA",header=True,index=False)
    #Barcodes uncertainty scores (RANK PLOT)
plt.figure(figsize=(20,10)).add_axes([0,0,1,1])
plt.plot(df_barcodes_uncertainty_scores["rank"].tolist(), df_barcodes_uncertainty_scores.uncertainty_score.tolist())
plt.xlabel('Rank')
plt.ylabel('Uncertainty score')
plt.savefig("{0}/Barcodes_uncertainty_scores.png".format(workspace_path),format="png")
plt.savefig("{0}/Barcodes_uncertainty_scores.svg".format(workspace_path),format="svg",dpi=600)
plt.savefig("{0}/Barcodes_uncertainty_scores.eps".format(workspace_path),format="eps",dpi=600)

plt.figure(figsize=(20,10)).add_axes([0,0,1,1])
plt.plot(df_barcodes_uncertainty_scores["rank"].tolist(), np.log10(df_barcodes_uncertainty_scores.uncertainty_score+(1e-16)).tolist())
plt.xlabel('Rank')
plt.ylabel('log(Uncertainty score)')
plt.savefig("{0}/log10_Barcodes_uncertainty_scores.png".format(workspace_path),format="png")
plt.savefig("{0}/log10_Barcodes_uncertainty_scores.svg".format(workspace_path),format="svg",dpi=600)
plt.savefig("{0}/log10_Barcodes_uncertainty_scores.eps".format(workspace_path),format="eps",dpi=600)

print("HMM imputations uncertainty scores all calculated at:")
print(datetime.now())
...
print(datetime.now())
#Computes the NAN euclidean distance ORIGINAL uncorrected cell genotype vs ALL reference lineages genotype)
    #only use position with certainty for lineage assignement
filtered_mtx_uncorrected_RM_ratio = mtx_uncorrected_RM_ratio
filtered_mtx_all_reference_lineage_genotypes = mtx_all_reference_lineage_genotypes
#filtered_mtx_uncorrected_RM_ratio[(abs(filtered_mtx_uncorrected_RM_ratio-0.5)<0.25)] = np.nan
#filtered_mtx_all_reference_lineage_genotypes[(abs(filtered_mtx_all_reference_lineage_genotypes-0.5)<0.25)] = np.nan
del mtx_uncorrected_RM_ratio
...
try:
    mtx_dist_org_genotype_vs_all_reference_lineages= pd.read_csv("{0}/mtx_dist_org_genotype_vs_all_reference_lineages.csv".format(workspace_path),sep="\t",header=None).to_numpy()
except:
    #mtx_dist_org_genotype_vs_all_reference_lineages = nan_euclidean_distances(filtered_mtx_uncorrected_RM_ratio, filtered_mtx_all_reference_lineage_genotypes)
    #Calculate NaN euclidean distance manually
    init_mtx_dist_org_genotype_vs_all_reference_lineages = np.zeros((nb_cells,np.shape(filtered_mtx_all_reference_lineage_genotypes)[0]))
    mtx_dist_org_genotype_vs_all_reference_lineages = Array('d', nb_cells*np.shape(filtered_mtx_all_reference_lineage_genotypes)[0])
    np_mtx_dist_org_genotype_vs_all_reference_lineages = np.frombuffer(mtx_dist_org_genotype_vs_all_reference_lineages.get_obj()).reshape((nb_cells,np.shape(filtered_mtx_all_reference_lineage_genotypes)[0]))
    np.copyto(np_mtx_dist_org_genotype_vs_all_reference_lineages, init_mtx_dist_org_genotype_vs_all_reference_lineages)
    #compute distances
    def compute_distances_org_cell_vs_reflineages(id_current_cell):
        global mtx_dist_org_genotype_vs_all_reference_lineages
        np_mtx_dist_org_genotype_vs_all_reference_lineages = np.frombuffer(mtx_dist_org_genotype_vs_all_reference_lineages.get_obj()).reshape((nb_cells,np.shape(filtered_mtx_all_reference_lineage_genotypes)[0]))
        current_barcode_indexes_non_na_positions = extend(np.argwhere(~np.isnan(filtered_mtx_uncorrected_RM_ratio[id_current_cell,:])))
        try:
            np_mtx_dist_org_genotype_vs_all_reference_lineages[id_current_cell,:] = np.sqrt(np.shape(filtered_mtx_uncorrected_RM_ratio)[1]/len(current_barcode_indexes_non_na_positions))*(euclidean_distances(filtered_mtx_uncorrected_RM_ratio[id_current_cell,current_barcode_indexes_non_na_positions].reshape(1, len(current_barcode_indexes_non_na_positions)),filtered_mtx_all_reference_lineage_genotypes[:,current_barcode_indexes_non_na_positions]))
        except:
            print(len(current_barcode_indexes_non_na_positions))
            print(fdsfs)
            print("filtered_mtx_uncorrected_RM_ratio[id_current_cell,current_barcode_indexes_non_na_positions][0:5]:")
            print(filtered_mtx_uncorrected_RM_ratio[id_current_cell,current_barcode_indexes_non_na_positions][0:5])
            print("filtered_mtx_all_reference_lineage_genotypes[:,current_barcode_indexes_non_na_positions][0:5]:")
            print(filtered_mtx_all_reference_lineage_genotypes[:,current_barcode_indexes_non_na_positions][0:5])
            print("len(current_barcode_indexes_non_na_positions):")
        if id_current_cell%100==0:
            print("Distances between standard cell genotype and reference genotypes computed for barcode {0}!".format(id_current_cell))
    # start pool of parallel worker processes
    with closing(Pool(processes=nb_cpus)) as pool:
        pool.map(compute_distances_org_cell_vs_reflineages, np.arange(nb_cells))
        pool.terminate()
    mtx_dist_org_genotype_vs_all_reference_lineages = np_mtx_dist_org_genotype_vs_all_reference_lineages
...
mtx_org_gen_vs_independent_batches_random_subsample_min_score = Array('d', nb_cells*nb_subsampling)
np_mtx_org_gen_vs_independent_batches_random_subsample_min_score = np.frombuffer(mtx_org_gen_vs_independent_batches_random_subsample_min_score.get_obj()).reshape((nb_cells,nb_subsampling))
np.copyto(np_mtx_org_gen_vs_independent_batches_random_subsample_min_score, init_mtx_org_gen_vs_independent_batches_random_subsample_min_score)

    #p-values computation
def original_genotype_vs_subsamples_lineage_assignment(id_subsample):
    global mtx_org_gen_vs_independent_batches_random_subsample_min_score
    np_mtx_org_gen_vs_independent_batches_random_subsample_min_score = np.frombuffer(mtx_org_gen_vs_independent_batches_random_subsample_min_score.get_obj()).reshape((nb_cells,nb_subsampling))
    ...
    mtx_dist_org_genotype_vs_subsample = mtx_dist_org_genotype_vs_all_reference_lineages[:,np.random.randint(4489, nb_rows_all_genotype_df, 4489).tolist()]
    np_mtx_org_gen_vs_independent_batches_random_subsample_min_score[:,id_subsample] = pd.DataFrame(mtx_dist_org_genotype_vs_subsample).min(axis=1)
    if id_subsample%nb_cpus==0:
        print("Distances between uncorrected genotype and random reference genotypes from subsample {0} computed (out of {1} subsamples)!".format(id_subsample+1,nb_subsampling))

# start pool of parallel worker processes
with closing(Pool(processes=nb_cpus)) as pool:
    pool.map(original_genotype_vs_subsamples_lineage_assignment, np.arange(nb_subsampling))
    pool.terminate()
mtx_org_gen_vs_independent_batches_random_subsample_min_score = np_mtx_org_gen_vs_independent_batches_random_subsample_min_score

init_arr_best_matches_org_genotype_vs_subsample_pvals = np.ones((nb_cells,1))
arr_best_matches_org_genotype_vs_subsample_pvals = Array('d', nb_cells)
np_arr_best_matches_org_genotype_vs_subsample_pvals = np.frombuffer(arr_best_matches_org_genotype_vs_subsample_pvals.get_obj()).reshape((nb_cells,1))
np.copyto(np_arr_best_matches_org_genotype_vs_subsample_pvals, init_arr_best_matches_org_genotype_vs_subsample_pvals)

def get_pvals_original_genotype_vs_subsamples_lineage_assignment(id_cell):
    global arr_best_matches_org_genotype_vs_subsample_pvals
    np_arr_best_matches_org_genotype_vs_subsample_pvals = np.frombuffer(arr_best_matches_org_genotype_vs_subsample_pvals.get_obj()).reshape((nb_cells,1))

    #original distance with best hit
    v_org_min_dist = df_best_match_org_cell_gen_vs_batch1.min_dist.tolist()[id_cell]
    np_arr_best_matches_org_genotype_vs_subsample_pvals[id_cell,0] = np.nansum(mtx_org_gen_vs_independent_batches_random_subsample_min_score[id_cell] <= v_org_min_dist)/(nb_subsampling)
    if id_cell%200==0:
        print("Min dist for cell {0} is {1} and p-value is {2}".format(lst_cells[id_cell],v_org_min_dist,np_arr_best_matches_org_genotype_vs_subsample_pvals[id_cell,0]))
        print("P-value lineage assignment computed for uncorrected genotype of cell {0} out of {1}!".format(id_cell+1,np.shape(df_best_match_org_cell_gen_vs_batch1)[0]))
# start pool of parallel worker processes
with closing(Pool(processes=nb_cpus)) as pool:
    pool.map(get_pvals_original_genotype_vs_subsamples_lineage_assignment, np.arange(np.shape(df_best_match_org_cell_gen_vs_batch1)[0]))
    pool.terminate()

arr_best_matches_org_genotype_vs_subsample_pvals = np_arr_best_matches_org_genotype_vs_subsample_pvals

for i in np.arange(np.shape(arr_best_matches_org_genotype_vs_subsample_pvals)[0]):
    df_best_match_org_cell_gen_vs_batch1.at[lst_cells[i],"pvalue"] = arr_best_matches_org_genotype_vs_subsample_pvals[i,0]

df_best_match_org_cell_gen_vs_batch1.to_csv("{0}/df_best_match_org_cell_gen_vs_batch1.csv".format(workspace_path), sep='\t',na_rep="NA",header=True,index=False)
print("P-values for lineage assignment of the uncorrected genotypes computed!")
print("End time of the computation of the NaN distances (org uncorrected vs batch1):")
print(datetime.now())
